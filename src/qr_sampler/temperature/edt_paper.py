...
Port notes (documented deviations):

- The repo-wide guardrail box clamps T to ``[0.3, 2.2]`` AFTER the formula,
  like every other strategy. The paper lets ``T -> 0`` for confident
  tokens; under this port those tokens sample at ``T = 0.3``. With the
  paper's ``T0 = 0.6`` the effective range is ``[0.3, 0.6]``.
...
- The paper pairs no truncation sampler. This strategy emits **no**
  per-token ``min_p`` diagnostic, so the selector-level ``min_p_base``
  applies (default 0.0 = none — the paper-faithful configuration).
